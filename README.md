...
現時点では、既存の、部分的に完成しているソフトウェアプロジェクトを保守するための `project-maintenance` スキルと、ステージ済み差分からコミットメッセージを作る `git-commit-message` スキルを収録しています。
...
Git の index に add 済みの差分だけを読み取り、リポジトリの最近のコミット傾向も参考にして、簡潔なコミットメッセージを生成するためのスキルです。

- `git-commit-message/SKILL.md`: スキル本体
- `git-commit-message/agents/openai.yaml`: エージェント向け表示名・説明・既定プロンプト
...
- `$git-commit-message`: `git add` 済みの差分を読んでコミットメッセージ案を作る
